...
public class JabiSurfingBot extends AdvancedRobot {

	private Map<String, Long> enemies = new ConcurrentHashMap<>();

	public static int BINS = 5;
	...
	public void onScannedRobot(ScannedRobotEvent e) {
		...
		enemies.put(e.getName(), System.currentTimeMillis());
		enemies.entrySet().stream()
			.filter(entry -> System.currentTimeMillis() - entry.getValue() > 1000)
			.forEach(entry -> enemies.remove(entry.getKey()));

		if (enemies.size() > 1 && e.getDistance() > 500) {
			scan();
		}
	...
	private void fireBullet(ScannedRobotEvent e) {
		...
		if (gunMove > 180) gunMove = gunMove - 360;
		if (gunMove < -180) gunMove = gunMove + 360;
		if (Math.abs(gunMove) > 1) {
			if (Math.abs(gunMove) > 18) {
				turnGunLeft(gunMove > 0 ? 18 : -18);
				return;
			}
			turnGunLeft(gunMove);
		}
